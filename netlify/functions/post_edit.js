...
exports.handler = async (event) => {
  ...
  try {
    ...
    const s = getSql();
    const rows = await s`SELECT id, user_id FROM posts WHERE id = ${postId} LIMIT 1`;
    if (!rows.length) return json(404, { error: "Post not found" });
    if (!isAdmin(user) && rows[0].user_id !== user.id) return json(403, { error: "Forbidden" });
    await s`UPDATE posts SET body = ${String(body)}, edited_at = NOW() WHERE id = ${postId}`;
